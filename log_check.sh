...
/disk[1-9][0-9]*.*(media is not present|device\/channel is not attached)|msdosfs_fat_uninit_vol: error 6 from msdosfs_fat_cache_flush|fseventsd.*disk logger.*failed to open output file.*No such file or directory|Volumes.*fseventsd out of sync with volume|jnl.*disk.*(replay_journal: from|journal replay done)|journal start\/end pointers reset/ {
	last_unplug_error=0;
